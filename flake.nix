      ...
      overlay = final: prev: {
        ...
        tws = with final; stdenv.mkDerivation {
          ...
          src = final.fetchurl {
            ...
            hash = "sha256-i9umdEB/a04MKgWsROBGShiKtybxd8x5nD7pLrl2L+I=";
          };
          ...
          installPhase = ''
            ...
            tail --bytes +17982 "$src" 2> /dev/null | head --bytes 218978622 > install4j/0.dat

            # Extract the installer and jre.
            tail --bytes 95482765 "$src" > sfx_archive.tar.gz
            ...
            cd install4j && \
              ...
              INSTALL4J_JAVA_HOME="$out/jre" \
                $out/jre/bin/java \
                  -Djava.ext.dirs="$out/jre/lib/ext:$out/jre/jre/lib/ext" \
                  -DjtsConfigDir="/home/jts" \
                  -classpath "i4jruntime.jar:launcher0.jar" \
                  install4j.Installer433163042 "-q" "-dir" "$out"
